...
**Intepretation:** Calories vary so widely across recipes that it is highly uncommon for more than one recipe to have the same number of calories, especially with such a sample sample size. It is recommeneded that this type of graph not be used on these data in the future. 

#### Scatterplot 1

What if we now wanted to know if there is a relationship between the amount of protein in a recipe and the amount of carbs. 
...
One may logically expect that a recipe that is high in fat will also be high in calories. Let's now explore if a relationship exists between these two variables in our data frame. 
...
What if we wanted to get a more visual sense of the five number summaries for fat, but fat across the levels of calorie rankings in the recipes. That is, how does the distribution of fat vary between recipes that are consider HIGH calorie and recipes considered LOW calorie? A Box plot is a great way to do that! Box plots also have the ability to see if there are any statistical outliers in the distribution of the data. 
...
``` {r macros side-by-side bar plot}
...
Now, what if I want to know the min and max protein for all of the recipes that contain feta, because maybe I am especially conscious of my protein intake. The following function will help us summarize those numeric data.

Next, what if I'd like to graphically see the min and max amount of protein, carbs, and fats, aggregated across all Greek recipes that contain feta. The code below will produce that display for us. 
